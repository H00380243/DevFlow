## Goal
- Build DemandFlow (智能需求交付系统) — complete all 23 features across 7 milestones, currently on Worker cycle for F005 (状态变更指令系统).
...
### In Progress
- **F005 (状态变更指令系统): FAILING** — Orient step pending
  - Dependencies: F004 ✓
  - SRS Trace: FR-004a
...
## Next Steps
1. Start F005 Orient → Bootstrap → Config Gate
2. F005 Feature Detailed Design via SubAgent
3. F005 TDD Red-Green-Refactor cycle
4. F005 Quality Gates, ST, Inline Check, Persist
5. Continue F006–F023

## Critical Context
- Progress: 4/23 features passing; Next: F005
- Critical path: F001→F002→F003→F004→F007→F008→F009→F010→F011
- 23 features total, 7 milestones
- F005 key classes: `CommandParser`, `ConfirmCommand`, `RejectCommand`, `PermissionChecker`
- F005 contract C-003: `POST /api/commands` accepts Command, returns `{status, message}`
- F005 SRS FR-004a: confirm/reject commands with permission validation
- Git HEAD: `3c4413b` (docs: update feature-list.json and task-progress.md)

## Relevant Files
- `docs/plans/2026-07-04-demandflow-srs.md` — Approved SRS (21 FRs, 11 NFRs); FR-004a is F005's srs_trace
- `docs/plans/2026-07-04-demandflow-design.md` — Approved Design; §2.3 (状态变更指令系统), §4.2 (API contracts C-003)
- `docs/plans/2026-07-04-demandflow-ats.md` — Approved ATS
- `feature-list.json` — Task inventory (F001-F004 passing, F005 failing)
- `task-progress.md` — Progress log (4/23, last: F004, next: F005)
...
- `RELEASE_NOTES.md` — Updated with F001 + F002 + F003 + F004
